...
export const getStats = async (req, res) => {
  try {
    ...
    const likesPerEvent = await pool.query(`
        ...
        GROUP BY e.id, e.title
      `);
    const mediaData = mediaPerEvent.rows.map((item) => ({
      ...item,
      media_count: Number(item.media_count),
    }));

    const likesData = likesPerEvent.rows.map((item) => ({
      ...item,
    ...
    res.json({
      ...
      popularEvent: popularEvent.rows[0] || null,
    });
  } catch (error) {
    res.status(500).json({
